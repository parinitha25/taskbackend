...
exports.register = function (req, res) {
  const { email, password, role, username, phoneNumber, gender } = req.body
  const User = new userData({ email, password, username, phoneNumber, gender, role: role || "basic" });
  User.save(function (err, data) {
    if (err) {
      return res.status(422).json({
        error: 'register not sucessfull, give some correct inputs'
      })
    }
    else {
...
exports.getallregister = function (req, res) {
  userData.find({}, (err, data) => {
    if (err) {
      ...
      return res.status(200).json({
        data,
        message: 'all user register sucessfully'
      })
    }
  })
}
